...
func (r *Registry) ListServices() (allServices map[string][]*registry.ServiceInstance, err error) {
	...
	mn, _, err = r.cli.cli.Catalog().Services(&api.QueryOptions{})
	if err != nil {
		return
	}
	allServices = make(map[string][]*registry.ServiceInstance)
	for k := range mn {
		ets, _, err := r.cli.cli.Health().Service(k, "", true, &api.QueryOptions{WaitTime: time.Second * 2})
		if err != nil {
			return nil, err
		}
		allServices[k] = r.cli.resolver(context.TODO(), ets)
	}
	return
}
